...
csr *setCSRMatrix( data *elements ){
	...
	matrix->rows = malloc( sizeof( int )*(elements->maxRow+1) );
...
void search( int row , int colunm , csr *matrix ){
	int i, startSearch, endSearch = matrix->amount;
	
	if( (row>matrix->maxRow)||(matrix->rows[row]==-1) ){
		printf("(%i,%i) = 0\n", row , colunm );
		return;
	}
	
	startSearch = matrix->rows[row];
	for( i=(row+1) ; i<=matrix->maxRow ; ++i ){
		if( matrix->rows[i] != -1 ){
			endSearch = matrix->rows[i];
			break;
		}
	}
